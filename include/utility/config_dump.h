...
#include "version.h" // BTANA_VERSION — generated by CMake from `git describe`
...
public:
          ...
          ctx_(cfg_dir_ ? cfg_dir_ : nullptr)
    {
        ...
        {
            cfg_dir_->cd();
            //  Stamp the build provenance first, so every output file's
            //  Config/ records the release this writer was built from
            //  (git describe at configure time; see version.h).
            add("btana_version", BTANA_VERSION);
        }
    }
