...
// ─────────────────────────────────────────────────────────────────────
//  Compute / serial-drain split (frame-level MT)
//
//  The weighted streaming score is the dominant per-frame serial cost and
//  has side effects (fills a QA hist, sets streaming-ring mask bits, emits
//  a trigger into the spill).  The split below lets the expensive scan run
//  on a worker thread while every side effect is deferred to a serial
//  drain in frame order:
//
