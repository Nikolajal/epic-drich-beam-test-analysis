...
| §ci-actions-node24 | Bump the remaining third-party actions to Node-24-compatible majors: `conda-incubator/setup-miniconda@v3`, `actions/upload-pages-artifact@v3`, `actions/deploy-pages@v4`.  GitHub deprecates Node 20 actions on **June 2026** with full removal in September 2026; new majors of these actions are not yet published as of 2026-05-27.  `actions/checkout` already bumped to `@v5`. | GitHub publishing Node-24 majors of those actions. |
| §ci-os-legs | Re-enable the per-OS build legs (`build-linux`, `build-macos`, `build-windows`) in [`.github/workflows/build.yml`](.github/workflows/build.yml).  All three jobs gated with `if: false` since 2026-05-27 due to ROOT-provisioning fragility (Ubuntu noble dropped apt `root-system`; conda-forge has no win-64 `root` recipe; Homebrew `root` churns periodically).  Workflow architecture preserved verbatim for reference. | Identifying a stable Windows ROOT provisioner; re-validating Linux conda-forge ROOT install timing fits CI budget; macOS Homebrew ROOT formula stabilising. |
| §macros-aclic-clean | Fix the example macros under `macros/examples/` so they compile cleanly under ACLiC (`scripts/check_macros.sh`).  As of 2026-05-27 only `elliptic_investigation.cpp` and `plot_fine_scan_evolution.cpp` pass; the rest fail with missing `#include`s (`<TH1.h>`, `<TH2.h>`, `<iostream>`, `<TTree.h>`, …) because they rely on cling's implicit pre-loaded ROOT headers.  Once every macro is ACLiC-clean, wire `scripts/check_macros.sh` into the CI workflow (gated on §ci-os-legs). | §ci-os-legs (no OS build legs ⇒ nowhere to run the check); decision on whether vestigial macros (`test_acceptance2`, `photon_number_new`, `photon_number_memory`) should be fixed or pruned. |
