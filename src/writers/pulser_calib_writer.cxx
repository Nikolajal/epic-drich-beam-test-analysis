    ...
    int max_spill)
{
    ...
    //  ── Phase 3a: write fine_calib.txt ────────────────────────────
    {
        std::ofstream out(fine_calib_path);
        if (!out)
        {
            mist::logger::error("(pulser_calib_writer) cannot open " + fine_calib_path + " for write");
            return;
        }
        long n_published = 0;
        long n_skipped = 0;
        for (const auto &pt : published)
        {
            ...
            //  determined by (chip, eo_channel) — empirically verified
            //  that each physical channel lives in exactly one FIFO,
            //  with 8 channels per FIFO inside one chip:
            //    fifo = 4·chip + ((eo_channel & 31) >> 3)
            ...
            //  v2 schema: p0 = a, p1 = -b, p2 = sigma.
            out << key << " "
                << static_cast<int>(CalibrationMethod::AlcorV2FitCalib) << " "
                << pt.a << " " << -pt.b << " " << pt.sigma << "\n";
            ++n_published;
        }
        mist::logger::info("(pulser_calib_writer) wrote " + std::to_string(n_published) +
                           " entries to " + fine_calib_path + "  (skipped " +
                           std::to_string(n_skipped) + " TDCs below threshold)");
    }
