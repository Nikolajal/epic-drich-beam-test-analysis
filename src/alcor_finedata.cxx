...
{
    const bool use_toml_format = path_has_toml_extension(filename);
    mist::logger::info(
        "(AlcorFinedata::write_calib_to_file) writing calibration to " +
        filename + (use_toml_format ? "  [TOML v3]" : "  [text v2]"));
    ...
    if (use_toml_format)
    {
        //  TOML v3 schema.  One [[entry]] table per GlobalIndex.
        //  Field meanings match the legacy line format exactly:
        ...
        for (const auto &[key, calib_params] : calibration_parameters)
        {
            const auto method_it = channel_calibration_method.find(key);
            const auto method = (method_it != channel_calibration_method.end())
                                    ? method_it->second
                                    : default_calibration_method;
            calib_file << "[[entry]]\n"
                       ...
                       << "a       = " << calib_params[0] << "\n"
                       << "minus_b = " << calib_params[1] << "\n"
                       << "sigma   = " << calib_params[2] << "\n\n";
        }
        return;
    }

    //  Legacy text v2 — one "key method a minus_b sigma" line per entry.
    for (const auto &[key, calib_params] : calibration_parameters)
    {
        const auto method_it = channel_calibration_method.find(key);
        const auto method = (method_it != channel_calibration_method.end())
...
void AlcorFinedata::read_calib_from_file(const std::string &filename, bool clear_first, bool overwrites)
{
    ...
    if (use_toml_format)
    {
        //  TOML v3 reader.  Looks for `schema = "fine_calib.v3"` plus
        //  a flat `[[entry]]` array.  Tolerates the schema key being
        ...
        for (const auto &node : *entries)
        {
            ...
            if (calibration_parameters.count(key_u32) && !overwrites)
                continue;
            calibration_parameters[key_u32] = {
                static_cast<float>(*a),
                static_cast<float>(*minus_b),
                static_cast<float>(*sigma)};
            channel_calibration_method[key_u32] =
                static_cast<CalibrationMethod>(*method_in);
        }
        return;
    }

    //  Legacy text v2 reader.
    std::ifstream calib_file(filename);
    if (!calib_file)
    {
        mist::logger::warning(
            "(AlcorFinedata::read_calib_from_file) cannot open " + filename +
            " — skipping load calibration");
        return;
    }
    std::string line;
    while (std::getline(calib_file, line))
    {
        ...
        uint32_t key;
        int      method_int;
        float    a, minus_b, sigma;
        parser >> key >> method_int >> a >> minus_b >> sigma;
        if (calibration_parameters.count(key) && !overwrites)
            continue;
        calibration_parameters[key] = {a, minus_b, sigma};
