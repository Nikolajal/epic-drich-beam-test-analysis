...
public:
  ...
  streaming_framer(std::vector<std::string> filenames, uint16_t frame_size = _FRAME_SIZE_) : streaming_framer(filenames, "", "", frame_size) {}
  streaming_framer(std::vector<std::string> filenames, std::string trigger_config_file, uint16_t frame_size = _FRAME_SIZE_) : streaming_framer(filenames, trigger_config_file, "", frame_size) {}
  streaming_framer(std::vector<std::string> filenames, std::string trigger_config_file, std::string readout_config_file, uint16_t frame_size = _FRAME_SIZE_) : _frame_size(frame_size)
  {
    // Create streams
    ...
    for (const auto &current_filename : filenames)
    {
      ...
      if (!data_streams.back().is_valid())
        std::cerr << "[WARNING] Failed to open streamer: " << current_filename << std::endl;
    }
    ...
    //  Readout configuration initialization
  ...
  //  Getters
  //  --- Copied value
  alcor_spilldata get_spilldata() const { return spilldata; }
  //  --- Linked reference
  alcor_spilldata &get_spilldata_link() { return spilldata; }
  std::map<std::string, TH1 *> get_QA_plots() { return QA_plots; }

  //   Setters
  //  --- Copied value
  void set_spilldata(alcor_spilldata v) { spilldata = v; }
  //  --- Linked reference
  void set_spilldata_link(alcor_spilldata &v) { spilldata = v; }
...
{
  //  Clear previous spill data
  spilldata.clear();
  _current_spill++;
  bool current_spill_has_data = false;

  //  Reference to frame list in spilldata
  auto &frame_list = spilldata.get_frame_link();

  // --- start of framer trigger
  ...
  for (auto &current_stream : data_streams)
  {
    ...
    //  Process update
    cout << "\33[2K\r[INFO] Spill " << (int)_current_spill << " processing file: " << current_stream.get_filename() << std::flush;
    ...
    while (current_stream.read_next())
    {
      ...
      //  Skip if not tagged for readout
      ...
      {
        //  mark that the current spill has data
        current_spill_has_data = true;

        //  check the status of lanes and update masks
        auto current_fifo = current_data.get_fifo();
        //  --- update participants mask
      ...
      //  Assigning frame-related time in data
