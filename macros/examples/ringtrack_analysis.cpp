...
std::array<float, 2> time_cut_boundaries = {-50, +20};
//std::array<float, 2> time_cut_boundaries = {-120, -50};
...
void ringtrack_analysis(std::string data_repository, std::string run_name, int max_frames = 1000000, int first_event = 0)
{
    ...
    int capped_end = min((long long)first_event + max_frames, n_frames);
    auto all_frames = capped_end;

    // -------------------------------------------------------------------------
    //  >>> SELECTION SETTINGS — change these lines <<<
    // -------------------------------------------------------------------------

    // DESPERATE SHIFT TEST
    int event_shift = 0;  // try 0, +1, -1, +2, -2, ...

    bool apply_multiplicity_cut = false;
    bool apply_radial_cut = false;
    bool require_single_track = true;
    bool require_multi_track = false;

  

    // se true: nel display vengono aggiunte a g_display_intercepts_multi TUTTE le
    // tracce dell'evento (non solo quella con chi2 migliore)
    bool display_all_tracks = true;

    // taglio sul numero di hit per il display
    // "none"    → nessun taglio
    // "greater" → n_hits > display_nhits_min
    // "range"   → display_nhits_min <= n_hits <= display_nhits_max
    std::string display_nhits_cut_mode = "range";
    int display_nhits_min = 0;
    int display_nhits_max = 5;

    int n_display_events = 100000;


    bool apply_theta_phi_cut = false;
    bool apply_angle_xy_cut = false;
    float theta_min = 0.001;
    float theta_max = 0.1;
    float phi_min = -3.14;
    float phi_max = 3.14;
    ...
    TCutG *cutg2 = make_cutg_rect(-35, +35, -35, +35);
    ...
    for (int i_frame = first_event; i_frame < all_frames; ++i_frame)
    {
        ...
        if (default_hardware_trigger)
        {
            for (auto current_hit = 0; current_hit < recotrackdata->get_recodata().size(); current_hit++)
            {
                ...
                if (time_delta < time_cut_boundaries[0] || time_delta > time_cut_boundaries[1])
                    continue;
    ...
    const int d_intercept_nbins = 100;
    const float d_intercept_min = 0.;
    const float d_intercept_max = 50.;
    TH1F *h_d_intercept = new TH1F("h_d_intercept",
                                   "Distance intercept-ring center at dRICH;d_{intercept} (mm);counts",
                                   d_intercept_nbins, d_intercept_min, d_intercept_max);
    TH2F *h_deltaR_vs_d_intercept = new TH2F("h_deltaR_vs_d_intercept",
                                             ...
                                             d_intercept_nbins, d_intercept_min, d_intercept_max, 500, -20., 20.);
    TH2F *h_ring_R_vs_d_intercept = new TH2F("h_ring_R_vs_d_intercept",
                                             ...
                                             d_intercept_nbins, d_intercept_min, d_intercept_max, 200, 30, 130);
    ...
    // intercette da eventi single track
    TGraph *g_display_intercepts_single = new TGraph();
    g_display_intercepts_single->SetName("g_display_intercepts_single");
    g_display_intercepts_single->SetTitle(Form("Intercepts - single track events (first %d selected events);x (mm);y (mm)", n_display_events));

    // intercette da eventi multi track (tutte le tracce dell'evento)
    TGraph *g_display_intercepts_multi = new TGraph();
    g_display_intercepts_multi->SetName("g_display_intercepts_multi");
    g_display_intercepts_multi->SetTitle(Form("Intercepts - multi track events (first %d selected events);x (mm);y (mm)", n_display_events));

    int n_display_filled = 0;

    TH2F *h_intercept_drich_zero_hits = new TH2F("h_intercept_drich_zero_hits",
                                                 "Track intercept at dRICH (0 hit events);x (mm);y (mm)",
                                                 120, -60., 60., 120, -60., 60.);
    ...
    for (int i_frame = first_event; i_frame < all_frames; ++i_frame)
    {
        ...
        if (default_hardware_trigger)
        {
            ...
            if (!require_single_track && recotrackdata->n_recotrackdata() < 1)
                continue;
            if (require_multi_track && recotrackdata->n_recotrackdata() < 2)
                continue;
            ...
            {
                if (track_angle_x < angle_x_min || track_angle_x > angle_x_max)
                    continue;
                if (track_angle_y < angle_y_min || track_angle_y > angle_y_max)
                    continue;
            }

            // -----------------------------------------------------------------
            // FIX 1: save ALL track-frame quantities BEFORE overwriting the
            //         buffer with GetEntry(hit_frame).
            // -----------------------------------------------------------------
            int   saved_n_tracks    = recotrackdata->n_recotrackdata();
            float saved_ix_scint    = plane_x + slope_x * z_scint;
            float saved_iy_scint    = plane_y + slope_y * z_scint;

            // salva le intercette di TUTTE le tracce dell'evento per il display
            std::vector<std::pair<float,float>> all_track_intercepts;
            if (display_all_tracks)
            {
                for (int i = 0; i < (int)recotrackdata->n_recotrackdata(); i++)
                {
            ...
            // -----------------------------------------------------------------
            // SHIFT: load hit frame (may be the same frame when event_shift==0)
            // -----------------------------------------------------------------
            int hit_frame = i_frame + event_shift;
            if (hit_frame < 0 || hit_frame >= all_frames)
                continue;

            recotrackdata_tree->GetEntry(hit_frame);
            auto hit_frame_trigger = recotrackdata->get_trigger_by_index(0);
            if (!hit_frame_trigger)
                continue;

            // --- loop over hits (hit-frame buffer, hit-frame trigger) ---
            ...
            h_n_selected_hits_vs_multiplicity->Fill(saved_n_tracks, selected_points.size());
            ...
            if (selected_points.size() == 0)
                h_intercept_drich_zero_hits->Fill(ix_drich, iy_drich);

            // -----------------------------------------------------------------
            // display
            // -----------------------------------------------------------------
            if (n_display_filled < n_display_events)
            {
                // taglio sul numero di hit per il display
                int n_hits = (int)selected_points.size();
                ...
                {
                    // single track: solo la best
                    if (saved_n_tracks == 1)
                    {
                        g_display_intercepts_single->AddPoint(ix_drich, iy_drich);
                    }
                    // multi track: tutte le tracce dell'evento
            ...
            // --- fit only if enough hits ---
            if (selected_points.size() > 4 &&
                (!apply_multiplicity_cut || fabs((int)selected_points.size() - 15) < 2))
            {
    ...
    // =========================================================================
    //  THIRD LOOP — events with streaming trigger but no reconstructed track
    // =========================================================================
    ...
    for (int i_frame = first_event; i_frame < all_frames; ++i_frame)
    {
        ...
        if (i_frame % 10000 == 0)
            bar_notrack.update(i_frame, all_frames);
        if (recotrackdata->is_start_of_spill())
            continue;

        if (recotrackdata->n_recotrackdata() != 0)
            continue;
    ...
    TNamed("display_all_tracks", Form("%i", display_all_tracks)).Write();
    TNamed("display_nhits_cut_mode", display_nhits_cut_mode.c_str()).Write();
    TNamed("display_nhits_min", Form("%d", display_nhits_min)).Write();
    TNamed("display_nhits_max", Form("%d", display_nhits_max)).Write();
    TNamed("n_display_events", Form("%d", n_display_events)).Write();
    ...
    {
        int mult = b - 1;
        int n_ev = h_event_multiplicity->GetBinContent(b);
        n_eventi_multi += n_ev;
