...
void cross_talk_treatment(std::string data_repository, std::string run_name, int max_frames = 10000000)
{
    ...
    for (int i_frame = 0; i_frame < all_frames; ++i_frame)
    {
        ...
        {
            if (recodata->is_afterpulse(i)) continue;
            ...
            {
                if (j == i) continue;
                if (recodata->is_afterpulse(j)) continue;

                const float dt = recodata->get_hit_t(j) - ti;
                if (dt < kElecDtLo || dt >= kDtMax) continue;
                ...
                if (is_phys) h_phys_dt->Fill(dt);
                if (is_elec) h_elec_dt->Fill(dt);
    ...
    auto draw_window = [](float lo, float hi, int color, const char *label)
    {
        ...
        l1->SetLineColor(color); l1->SetLineStyle(2); l1->Draw();
        l2->SetLineColor(color); l2->SetLineStyle(2); l2->Draw();
        TLatex *tex = new TLatex(lo, gPad->GetUymax() * 0.9, label);
        tex->SetTextColor(color); tex->SetTextSize(0.03); tex->Draw();
    };
    ...
    c->cd(1); gPad->SetLogy();
    h_phys_dt->SetLineColor(kBlue + 1); h_phys_dt->SetLineWidth(2);
    h_phys_dt->Draw("HIST");
    draw_window(kPhysCtLoNs,   kPhysCtHiNs,       kRed,    "signal");
    draw_window(kSidebandLoNs, kPhysSidebandHiNs, kGreen+2, "sideband");

    c->cd(2); gPad->SetLogy();
    h_elec_dt->SetLineColor(kOrange + 7); h_elec_dt->SetLineWidth(2);
    h_elec_dt->Draw("HIST");
