...
std::array<float, 2> time_cut_boundaries = {-40., 50.};
...
void photon_number(std::string data_repository, std::string run_name, int max_frames = 10000000)
{
    ...
    h_r_distribution_full->Scale(1., "width");
    ...
    h_r_distribution_included->Scale(1., "width");
    ...
    h_r_distribution_excluded->Scale(1., "width");
    ...
    TF1 *f_fit_r_dist = new TF1("f_fit_r_dist", "[0]*TMath::Gaus(x,[1],[2],true) + [3]*TMath::Gaus(x,[1],[4],true)  + pol3(5)");
    f_fit_r_dist->SetParameter(0, h_r_distribution_excluded->GetMaximum());
    f_fit_r_dist->SetParameter(1, h_r_distribution_excluded->GetMean());
    f_fit_r_dist->SetParameter(2, h_r_distribution_excluded->GetRMS());
    f_fit_r_dist->SetParameter(3, h_r_distribution_excluded->GetMaximum());
    f_fit_r_dist->SetParameter(4, h_r_distribution_excluded->GetRMS());
    new TCanvas();
    h_r_distribution_full->Fit(f_fit_r_dist);
    new TCanvas();
    h_r_distribution_included->Fit(f_fit_r_dist);
    new TCanvas();
    h_r_distribution_excluded->Fit(f_fit_r_dist);
}