...
std::vector<trigger_config> trigger_conf_reader(std::string config_file)
{
  ...
  //  Read the trigger configurations
  ...
  while (std::getline(infile, line))
  {
    ...
    trigger_config current_trigger_config;
    if (iss >> current_trigger_config.name >> current_trigger_config.delay >> current_trigger_config.device)
      triggers.push_back(current_trigger_config);
    else
      std::cerr << "[WARNING][trigger_config_reader] could not parse line: " << line << std::endl;
  }
...
int TRIGGER1_device = 197;
int TRIGGER2_device = 0;
int TRIGGER3_device = 0;
int TRIGGER0_offset = 112;
int TRIGGER1_offset = 122;
int TRIGGER2_offset = 0;
int TRIGGER3_offset = 0;
#endif
...
void lightwriter(std::vector<std::string> filenames, std::string outfilename, std::string fineoutfilename, unsigned int max_spill = 50, bool verbose = false)
{
...
void lightwriter(std::string runname, unsigned int max_spill = kMaxUInt, bool verbose = false)
{
  lightwriter("./Data/" + runname + "/", "./Data/" + runname + "/lightdata.root", "./Data/" + runname + "/finedata.root", max_spill, verbose);
}
