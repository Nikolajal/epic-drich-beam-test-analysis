...
[Feature · F1.0 L1.90 I3 · CAMPAIGN · READY · C++   · P 2.07]  Hough threshold → DCR-dependent.  Replace the fixed `hough_threshold_fraction × N_active_cherenkov` floor (lightdata_writer.cxx:908, blind to dark rate) with a DCR-adaptive floor derived from the first-frames noise model already in score.h: E_dark = Σ_c m_c (expected DCR hits/window), min_active = max(abs_floor, E_dark + k·√E_dark) — a Poisson n_σ over the dark floor, mirroring stage 1's `n_σ = (S−E[S])/σ_S`.  Add a `hough_n_sigma_dcr` knob; retire `hough_threshold_fraction`.  Motivation: on noisy runs (σ_S≳3000, e.g. 20260614-132826) the fixed fraction floods with DCR-coincidence solo "rings" and no constant bump separates signal from noise (raising it halves real dual-ring yield while solos stay junk).  Validate streaming-noisy vs dense regimes; confirm dual-ring yield preserved.  Design note: include/triggers/streaming/DISCUSSION.md → "Hough threshold — make it DCR-dependent".
...
[Schema · F1.0 L2.00 I2 · CAMPAIGN · READY · C++   · P 2.00]  GlobalIndex reserved bits [29,30] → 2-bit `readout_mode` enum {LET=0, ToT, ToT2, SR}.  Repurpose the strict-zero "schema-version" reserved field as the ALCOR readout mode — it's exactly a 2-bit enum and tells a reader how to interpret the coarse/fine payload (timing chip = LET, data chips = ToT), self-describing without a config side-lookup.  Mode is per-channel-constant so it doesn't break key/`global_channel` semantics.  **LET=0** keeps every existing persisted index + `fine_calib.toml` `key` valid (reads as LET).  Touches global_index.h (kModeBits/kModeShift/kModeMask, `mode()` accessor, `ReadoutMode` enum, ctor mode arg), drops `reserved_bits_are_zero()` invariant + its test (add mode round-trip tests), `global_channel` keeps the mode bits (only TDC zeroed), decoder sets mode from the readout config.  **MAJOR / Schema** bump (packed layout is persisted as the calibration key).  Design note: include/utility/DISCUSSION.md → `global_index.h`.  Deferred until firmware mode-tagging is wired up.
...
[Patch · F0.0 L1.50 I3 · CAMPAIGN · READY · Conf  · P 2.00]  RANSAC finder — tune the scan for the 2026 far-arc regime to reach the physics expectation of ~≥1 Cherenkov ring per hardware trigger.  Today the 2026 per-trigger ring yield is LOW (most hardware-triggered frames reconstruct no ring), while a real beam trigger should almost always carry a ring.  Tune the RANSAC acceptance (`ransac_min_significance`, `ransac_min_inliers`, `min_visible_arc_frac`, `collection_radius`/inlier_band, r_min/r_max) plus recodata `arc_span_min_rad`, balancing yield against spurious far rings (the wide ±centre / large-r_max scan otherwise admits noise rings whose N_γ/ring collapses, ~0.5, χ²/ndf~100).  Iterate FAST with the `ransac_tune` harness (macros/utilities/ransac_tune.cpp — re-runs `find_rings_ransac` on an existing lightdata.root with CLI knobs, supports `--dt-scan` for the trigger→hit Δt + a trigger census; NO lightdata re-run needed).  Validate streaming-noisy (σ_S≳3000, e.g. 20260605-044003) vs dense regimes; verify the centre/R distribution tightens onto the real arc band.  Supersedes the old Hough-era threshold tuning (the `hough_*` knobs are now IGNORED).
...
[Bug · F1.0 L1.71 I2 · CAMPAIGN · READY · C++   · P 0.74]  Framer tree output is non-reproducible run-to-run — two IDENTICAL `lightdata_writer … --skip-stream-qa --threads 1` runs (framer-only, fully serial, zero post-framer code) produce different `lightdata` TTree bytes (same byte size, different content), in BOTH LET (op-mode 1) and ToT (op-mode 4).  The stable-sort at `parallel_streaming_framer.cxx:898-904` (sort by `(GlobalIndex, rollover, coarse, fine)`) is meant to make output schedule-independent but does NOT: some hit field/structure the sort key omits, or a merge-order residue, still varies.  Deterministic QA histograms ARE reproducible (ring geometry, counts, etc.) — only the raw serialized tree differs — so physics is unaffected, but it breaks the project bit-identity standard for the tree (and made the Stage-2 MT bit-identity gate fall back to comparing deterministic hists only).  NOT introduced by the ToT or MT work (reproduces in plain LET, framer-only).  Investigate: per-branch hash to localize which branch/field varies (dead_mask/participants_mask are std::map = ordered; hit vectors are sorted; suspect a non-keyed field or std::sort tie-break), then determine regression-vs-always-existed (the original 6eb1f8a MT was reportedly "verified bit-identical", suggesting a later regression).
...
[Patch · F1.0 L1.49 I1 · LATER    · READY · Doc   · P 0.40]  PDF-naming drift: util::qa::pdf_path (include/utility/qa_publish.h) now ignores its `order` argument and emits a plain `<name>.pdf` (no `NN_` numeric prefix), so CLAUDE_GUIDELINES §5 "the NN_ numeric prefix orders within a stage" is stale, and the `order` / `trg_order++` literals threaded through the lightdata QA call sites are dead args.  Update §5 + drop the dead args (filenames are name-unique — tot_spectrum / tot_vs_channel / dt_vs_tot_<trig> etc. — so no collision risk).
[Patch · F0.0 L0.30 I1 · CAMPAIGN · INVESTIGATE · C++   · P ?]  ToT pairing — confirm both TDC groups {0,1} AND {2,3} are wired in the hardware ToT readout config.  The framer pairs each group independently (key = channel<<1 | tdc>>1), so a pixel emits up to 2 paired ToT hits/pulse; if only one group carries real ToT data the other produces spurious pairs.  Validate against the offline study that set the ~1-2% orphan rate so the ToT spectrum normalization is sound.
```
