...
[Feature · F1.0 L1.50 I2 · LATER    · READY · C++   · P 1.10]  Streaming n_sigma over-firing: tune the per-run threshold (firing ~8× physical rate).  Over-firing inflates the serial per-frame QA body (per-trigger Δt / hitmaps / pair-Δt on every fired frame), which is now the Amdahl ceiling on the lightdata frame-level MT (DISCUSSION §2.7.1) — a tuned threshold shifts the run back into the parallel score/Hough passes and raises the speedup.  Was the PREREQ of the (now-shipped) MT row.
