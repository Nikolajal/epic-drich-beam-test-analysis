...
[Patch · F0.0 L1.30 I2 · CAMPAIGN · READY · Conf  · P 1.54]  Wide-arc Hough: retune acceptance floors for the far-off-centre regime.  On streaming-noisy runs (σ_S≳3000, e.g. 20260614-132826) the ±250 mm centre / 359 mm r_max accumulator admits many spurious wide rings — `ring_X/Y/R_first_hough` pile at the range edges.  Downstream recodata then fits one ring per hardware-trigger frame, so N_γ/ring collapses (~0.5, χ²/ndf~100).  Retune `min_ring_votes` / `hough_threshold_fraction` / `threshold_fraction` (and possibly raise `arc_span_min_rad` above 0.5) for the wide regime against a clean arc run; verify the centre/R distribution tightens onto the real arc band.  Thresholds currently inherit the near-origin tuning.
